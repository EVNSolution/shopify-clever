...
## Shopify 원본 데이터 읽기 전용 규칙

- K-food와 모든 dev store의 Shopify 주문·고객 데이터는 변경하지 않는 upstream
  정본이다.
- Orders 기능은 `read_orders`, `read_customers` query만 사용하며
  `orderUpdate`, `customerUpdate` 같은 Shopify mutation을 추가하지 않는다.
- 배송일, Area, 상태, 결제 방식 등 운영 보정값은 Delivery API를 통해 CLEVER DB에만
  저장한다. Shopify 원본으로 역기록하지 않는다.
- AppInstallation metafield는 출발지와 앱 환경설정처럼 앱이 소유한 설정에만 쓴다.
  주문·고객 운영 데이터를 저장하는 용도로 확장하지 않는다.
