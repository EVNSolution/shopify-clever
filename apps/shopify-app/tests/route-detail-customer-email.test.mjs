...
test("route detail customer email modal uses route-aware defaults and a compact empty layout", () => {
  ...
  assert.match(routeDetailSource, /gridTemplateColumns: "minmax\(0, 1fr\)"/);
  assert.match(routeDetailSource, /const customerEmailDialogStyle = \{[\s\S]*gridTemplateRows: "auto minmax\(0, 1fr\) auto"[\s\S]*overflow: "hidden"[\s\S]*width: "620px"/);
  assert.doesNotMatch(routeDetailSource, /repeat\(auto-fit, minmax\(min\(320px, 100%\), 1fr\)\)/);
