...
test("Drivers is a single operational driver list without vehicle or assignment sub-tabs", () => {
  ...
  const [, copyDownloadLinkBody = ""] = source.match(/const copyDownloadLink = async \(\) => \{([\s\S]*?)\n  \};/) ?? [];
  assert.match(copyDownloadLinkBody, /DRIVER_DOWNLOAD_LINK/);
  assert.doesNotMatch(copyDownloadLinkBody, /normalizeInvitePhone/);
  assert.doesNotMatch(copyDownloadLinkBody, /savedInvitePhone/);
  assert.doesNotMatch(copyDownloadLinkBody, /driverInviteFetcher\.submit/);
