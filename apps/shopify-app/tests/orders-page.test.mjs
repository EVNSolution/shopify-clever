...
test("Orders table keeps delivery state operational and payment state separate", () => {
  ...
  assert.match(ordersPageSource, /Note mentions pickup, but raw Order Type\/Pickup Day is missing/);
  assert.match(ordersPageSource, /Note mentions pickup, but raw Order Type is/);
  ...
  assert.match(ordersPageSource, /Raw Delivery Day missing/);
  ...
  assert.match(ordersPageSource, /Delivered source: Shopify fulfillment only; CLEVER driver status is not present/);
  ...
  assert.match(ordersPageSource, /formatInfoDetail\("CLEVER driver stop"/);
...
test("Orders inventory detail shows a printable product matrix without delta", () => {
  ...
  assert.match(inventoryDetailSource, /<article className="inventory-detail-order-card"/);
  ...
  assert.match(inventoryDetailSource, /\.inventory-detail-orders-head \{ border-bottom: 1px solid #111 !important; font-size: 12px !important; font-weight: 750 !important; line-height: 16px !important; margin-bottom: 8mm !important/);
  ...
  assert.match(inventoryDetailSource, /\.inventory-detail-order-card \{ -webkit-column-break-inside: avoid !important; border-top: 1px solid #c7c7c7 !important; break-inside: avoid !important; break-inside: avoid-page !important; display: block !important/);
  assert.match(inventoryDetailSource, /margin: 0 0 8mm !important/);
  assert.match(inventoryDetailSource, /\.inventory-detail-order-row \{ border-bottom: 1px solid #e5e7eb !important; font-size: 12px !important; line-height: 17px !important; padding: 4mm 0 !important; \}/);
  assert.match(inventoryDetailSource, /\.inventory-detail-order-items \{ break-inside: avoid !important; display: grid !important; font-size: 12px !important; gap: 1mm !important; line-height: 17px !important; padding: 3mm 0 4mm !important; page-break-inside: avoid !important; \}/);
