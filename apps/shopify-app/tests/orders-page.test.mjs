...
test("Orders inventory detail shows a printable product matrix without delta", () => {
  ...
  assert.match(inventoryDetailSource, /const orderViewColumns = "74px minmax\(0, 1fr\) 58px 76px 72px 118px 86px"/);
  ...
  assert.match(inventoryDetailSource, /\$\{order\.price\} · \$\{order\.payment\}/);
  ...
  assert.match(inventoryDetailSource, /const PRINT_CONTENT_HEIGHT_MM = 286/);
  ...
  assert.match(inventoryDetailSource, /getBoundingClientRect\(\)\.height/);
  ...
  assert.match(inventoryDetailSource, /\.inventory-detail-orders-list \{ display: grid !important; gap: 8mm !important; width: 100% !important; \}/);
  assert.match(inventoryDetailSource, /grid-template-columns: 18mm minmax\(0, 1fr\) 12mm 20mm 18mm 30mm 22mm !important/);
  assert.match(inventoryDetailSource, /\.inventory-detail-order-card \{ border-bottom: 1px solid #c7c7c7 !important; border-top: 1px solid #c7c7c7 !important; break-inside: avoid-page !important/);
  assert.match(inventoryDetailSource, /\.inventory-detail-order-row \{ border-bottom: 1px solid #e5e7eb !important; font-size: 15px !important; line-height: 21px !important; padding: 4mm 0 !important; \}/);
  assert.match(inventoryDetailSource, /\.inventory-detail-order-items \{ display: grid !important; font-size: 15px !important; gap: 1mm !important; line-height: 21px !important; padding: 3mm 0 4mm !important; \}/);
  assert.match(inventoryDetailSource, /box-sizing: border-box !important; display: block !important/);
  assert.match(inventoryDetailSource, /padding: 4mm 10mm 7mm !important; width: 210mm !important/);
