...
test("Routes page lists saved child routes as real route rows", () => {
  ...
  assert.match(routeListRowsSource, /function buildRouteChildRows\([\s\S]*groupAccentColor = null,[\s\S]*groupSummary = null,[\s\S]*routePlanById = new Map\(\),[\s\S]*\) \{/);
  ...
  assert.match(routeListRowsSource, /route: routePlan\.name \? getRouteGroupChildRouteName\(routeGroup, child, routePlan, index\) : "-"/);
...
test("Routes table exposes actual route metrics without parent group rows", () => {
  ...
  assert.match(routesPageSource, /translate\(language, "routes\.table\.stops"\)/);
...
test("Routes page keeps copied controls out while using checkbox route selection actions", () => {
  ...
  assert.match(routesPageSource, /minWidth: "1320px"/);
...
test("Routes table uses aligned CLEVER planning columns", () => {
  ...
  assert.match(routesPageSource, /translate\(language, "routes\.table\.name"\)/);
  assert.match(routesPageSource, /translate\(language, "routes\.table\.name"\)[\s\S]*translate\(language, "routes\.table\.status"\)[\s\S]*translate\(language, "routes\.table\.driver"\)[\s\S]*translate\(language, "routes\.table\.startTime"\)/);
  assert.match(routesPageSource, /translate\(language, "routes\.table\.stops"\)/);
  assert.match(routesPageSource, /translate\(language, "routes\.table\.totalItems"\)/);
  ...
  assert.match(routesPageSource, /translate\(language, "routes\.table\.created"\)/);
  ...
  assert.match(routeListRowsSource, /childRows: buildRouteChildRows\(routeGroup, children, groupAccentColor, groupSummary, routePlanById\)/);
...
test("Routes table renders the group marker inside the route name cell", () => {
  assert.match(routesPageSource, /import \{ createPortal \} from "react-dom"/);
  assert.doesNotMatch(routesPageSource, /const routeGroupMarkerHeaderCellStyle = \{/);
  assert.doesNotMatch(routesPageSource, /const routeGroupMarkerCellStyle = \{/);
  ...
  assert.doesNotMatch(routesPageSource, /<th aria-hidden="true"/);
  assert.match(routesPageSource, /<td style=\{routeNameCellStyle\}>[\s\S]*route\.groupAccentColor[\s\S]*route\.route[\s\S]*<\/td>/);
...
test("Route detail stages driver assignment for the global route draft save", () => {
  assert.doesNotMatch(routeDetailServerSource, /assignDeliveryRoutePlanDriver/);
  assert.doesNotMatch(routeDetailServerSource, /intent === "saveRouteDriver"/);
  assert.match(routeDetailSource, /buildRouteDriverOptions\(drivers, effectiveRoutePlan\?\.driver\)/);
  assert.match(routeDetailSource, /\{ id: "", label: "Unassigned" \}/);
  assert.doesNotMatch(routeDetailSource, /Invite pending/);
  assert.match(routeDetailSource, /driverId: driverId \|\| null/);
  assert.match(routeDetailSource, /driverId: routeRow\.driverId \?\? null/);
...
test("Route group detail keeps its own page instead of becoming a child route", () => {
  assert.doesNotMatch(routesPageSource, /groupsWithoutRoutes|routes\.group\.withoutRoutes/);
  ...
  assert.doesNotMatch(routeGroupDetailSource, /route_group_detail\.api\.raw|route_group_detail\.api\.list|JSON\.stringify\(data, null, 2\)/);
...
test("Route group detail hides scrollbar chrome without disabling horizontal scrolling", () => {
  assert.match(
    routeDetailSource,
    /className="route-group-detail-scroll" style=\{routesDetailTableFrameStyle\}/,
  );
  assert.match(
    routeDetailSource,
    /className="route-group-detail-scroll" style=\{\{ \.\.\.routeTimelineRowsStyle, minHeight: routeTimelineRowsMinHeight \}\}/,
  );
  assert.match(globalCssSource, /\.route-group-detail-scroll \{[\s\S]*scrollbar-width: none;[\s\S]*\}/);
  assert.match(globalCssSource, /\.route-group-detail-scroll::-webkit-scrollbar \{[\s\S]*display: none;[\s\S]*\}/);
});
...
test("Route detail renders a compact route overview panel with inline summary", () => {
  ...
  assert.match(routeDetailSource, /anchorX - width \+ ROUTE_TIMELINE_STOP_POPOVER_EDGE_INSET/);
  ...
  assert.match(routeDetailSource, /const handleRouteTimelineStopMouseLeave = \(stop\) => \{/);
  assert.match(routeDetailSource, /document\.addEventListener\("pointerdown", handleDocumentPointerDown\)/);
...
test("Route detail places stop and departure markers through MapLibre source layers", () => {
  ...
  assert.match(routeDetailSource, /fitRouteDetailMap\(mapRef\.current, maplibregl, routeMapFitLocations\)/);
...
test("Route detail only auto-fits the map on initial map readiness", () => {
  ...
  assert.match(routeDetailSource, /routeMapCenterRef\.current = routeMapFitLocations\.at\(-1\)\?\.coordinates \?\? routeMapCenter/);
...
test("Route detail renders route lines and a stop timeline below the map", () => {
  ...
  assert.doesNotMatch(routeDetailSource, />Vehicle<\/th>/);
  ...
  assert.doesNotMatch(routeDetailSource, /aria-label="Change route vehicle"/);
  ...
  assert.match(routeDetailSource, /setRouteStartTimeDraft\(buildRouteStartDraft\([\s\S]*routeRow\.scheduledStartTimeZone \?\? ianaTimezone/);
  assert.match(routeDetailSource, /routeRowId: routeRow\.id/);
  assert.match(routeDetailSource, /const targetRouteRowId = activeRouteSelector\?\.type === "startTime"[\s\S]*?activeRouteSelector\.routeRowId[\s\S]*?: currentRouteLineId/);
  assert.match(routeDetailSource, /scheduledStartAt: routeRow\.scheduledStartAt \?\? null/);
  assert.doesNotMatch(routeDetailSource, /formData\.set\("routePlanId", targetRoutePlanId\)/);
  ...
  assert.match(routeDetailSource, /wheelHintEnabled=\{isTrackingMapView \|\| !isRoutePolygonEditMode\}/);
...
test("Route detail opens the driver selector and removes the unfinished vehicle affordance", () => {
  ...
  assert.doesNotMatch(routeDetailSource, /handleOpenRouteSelector\("vehicle"/);
  assert.doesNotMatch(routeDetailSource, /Change route vehicle/);
...
test("Route detail page provides page navigation back to the route list", () => {
  ...
  assert.match(routeDetailSource, /const handleBackToRoutes = \(\) => \{/);
  assert.match(routeDetailSource, /setPendingRouteDraftHref\(href\)/);
  assert.match(routeDetailSource, /requestRouteNavigation\(routesListHref\)/);
  assert.match(routeDetailSource, /aria-label="Unsaved route changes"/);
  assert.match(routeDetailSource, /아직 남은 변경이 있습니다/);
  assert.match(routeDetailSource, /onClick=\{handleSaveRouteDraftAndLeave\}/);
  assert.match(routeDetailSource, /onClick=\{handleDiscardRouteDraftAndLeave\}/);
  assert.match(routeDetailSource, /navigateAfterRouteDraftSaveRef\.current = pendingRouteDraftHref \?\? routesListHref/);
  assert.match(routeDetailSource, /window\.addEventListener\("beforeunload", handleBeforeUnload\)/);
  assert.match(routeDetailSource, /onClick=\{handleBackToRoutes\}/);
...
test("Route detail can move between child routes in the same route group", () => {
  ...
  assert.match(routeDetailSource, /requestRouteNavigation\(routeGroupChildPath\(routeGroupId, routePlanId\)\)/);
...
test("child detail supports adding and reversing stops without refreshing over a draft", () => {
  ...
  assert.match(routeDetailSource, /aria-label="Add stops"[\s\S]*aria-modal="true"[\s\S]*aria-label="Available orders"/);
