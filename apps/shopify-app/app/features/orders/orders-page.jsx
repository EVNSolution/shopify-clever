...
export function getCreatedRouteDestination(intent, routeGroup) {
...
const ordersRefreshStatusStyle = {
  color: "#616161",
  fontSize: "12px",
  lineHeight: "18px",
  whiteSpace: "nowrap",
...
function OrdersPageContent({ loaderData }) {
  ...
  const handleCreateRoute = () => submitNewRoute("createRouteGroup");
  ...
  useEffect(() => {
    const completion = getOrdersRefreshCompletion({
      activeRequestId: activeOrdersRefreshRequestIdRef.current,
      data: ordersRefreshFetcher.data,
      fetcherState: ordersRefreshFetcher.state,
      handledRequestId: handledOrdersRefreshRequestIdRef.current,
    });
    if (!completion) return;

    handledOrdersRefreshRequestIdRef.current = completion.requestId;
  ...
  useEffect(() => {
    ...
    if (submittedRouteIntent === "createRouteGroup") {
      ...
      routeCreatePendingRef.current = false;
      setRouteCreatePending(false);
      const destination = getCreatedRouteDestination(submittedRouteIntent, createdRouteGroup);
      if (!destination) {
        setCreateRouteClientError(translate(language, "orders.routeActions.missingCreatedRoute"));
        return;
      }
