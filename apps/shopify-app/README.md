...
`npm run dev -- --store clever-test-syhae28n.myshopify.com` runs the dev app config with safe Delivery API defaults, without a `.env.local` file. `npm run dev:local` remains an alias for compatibility; see `../../docs/runbooks/shopify-app-dev.md`. Use `npm run deploy:prod`, `npm run deploy:dev`, or `npm run deploy:kfood` only after an explicit release decision.
