...
새 경로 생성과 기존 경로 추가에 모두 적용된다. 주문 조회 및 Shopify 원본은 유지한다.
Save 오류의 소유권 복구 및 그룹 Unassigned 계약은 별도 서버 저장소 범위다.
웹 검사만으로 기존 409 오류가 복구됐다고 보고하지 않는다.

## 후속 사용자 요구: 경로 생성 통일

사용자는 일반/그룹 경로를 구분하지 않고, 기존 그룹 생성을 `Create route` 하나로
제공하도록 요청했다. Orders의 별도 `Create group route` 버튼을 제거하고
남은 `Create route` / `경로 생성` 버튼을 기존 그룹 생성 API에 연결했다.
생성 완료 시 반환된 경로의 전체 경로 화면으로 이동한다.

Orders BFF에서도 일반 생성 API 분기를 제거했다. 이전 화면에서 제출하는
`createRoutePlan` 요청은 주문 동기화나 경로 생성 전에 새로고침 안내로 거절한다.
기본 생성 요청과 명시적 생성 요청은 모두 `createRouteGroup`을 사용한다.
기존 단일 경로 데이터·조회·상세 동작과 서버의 분할 저장 계약은 변경하지 않았다.
취소 행 유지·선택 차단 및 기존 경로에 주문 추가는 유지한다.

이 요구와 구현 결과를 이 사건 문서에 기록했다. 별도 control-plane/context 저장소는
현재 웹 수정 범위 밖이므로 변경하지 않았다. 아래 서버 전달문은 최초 조사 시점의 근거다.
...
최초 취소 주문 결함 수정 후 경로 생성을 통일하는 사용자 요구가 추가됐다.
서버 구현/복구 및 배포 여부는 별도 확인이 필요하다.
...
- 수정 전 새 회귀 테스트 4개 중 3개 실패: 일반 생성 버튼, 기본 생성 요청 및 이전 일반 생성 요청이
  각각 단일 경로 API를 사용함을 확인했다. 수정 후 4/4 통과했다.
- Orders 생성·선택·BFF 관련 테스트: 121/121 통과.
- 전체 `npm test`: 736개 중 735개 통과. 변경하지 않은 `compliance-webhook.test.mjs:165`의
  100ms 제한 검사가 시간 초과했다. 해당 파일을 단독 재실행해 6/6 통과했다.
- 주문 필터 및 번역 테스트: 27/27 통과.
- build, typecheck, public URL guard, 변경 파일 ESLint, `git diff --check`: 통과.
- 운영 배포 및 실제 경로 생성·저장은 실행하지 않았다.
